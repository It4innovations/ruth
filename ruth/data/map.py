...
def segment_weight(n1, n2, data):
    ...
    # print("Seg length: ",data['length'])
    return float(data['length']) + float(f"0.{n1}{n2}")


def segment_weight_speed(n1, n2, data):
    assert "speed_kph" in data, f"Expected the 'speed' of segment to be known. ({n1}, {n2})"
    maxspeed = data.get('speed_kph', 110)  # Default maximum speed of 110 if 'maxspeed' attribute is missing
    # print("Seg_speed",data['speed_kph'])
