...
from ruth.utils import osm_route_to_segments, route_to_osm_route, TimerSet, riffle_shuffle
...
class Simulator:
    ...
    def simulate(self,
        ...
        while self.current_offset is not None:
            ...
            logger.info(f"{step}. offset: {offset}, active: {len(allowed_vehicles)}, duration: {step_dur / timedelta(milliseconds=1)} ms")
        ...
        #cProfile result collection BEGIN
        pr.disable()
        ...
        ps.print_stats()
        print(s.getvalue())
        #cProfile result collection END        
